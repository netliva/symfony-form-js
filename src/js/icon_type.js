...
(function( $ ) {
	...
	$.fn.netlivaIconField = function(settings)
	{
		settings = $.extend({
			icons : {
			...
			groups : {
				"solid"   : {prefix: "fas", title: "Yekpare"},
				"regular" : {prefix: "far", title: "Düzenli"},
				"brands"  : {prefix: "fab", title: "Markalar"},
			},
		}, settings);
		...
		var modal = {
			open: function (options){
				...
				if (!$("#netliva_icon_modal").length) modal.create();
				...
				if (options.buttons) modal.create_buttons(options.buttons);
			},
			close: function () {
		...
		$form_row.find("button").click(function () {
			modal.open({
				title:"İcon Seçimi Yapın",
				content:'<div><input id="netlivaIconFinderInput" type="text" class="form-control" placeholder="İcon Ara" /></div><ul class="list-unstyled"></ul>',
				init: function () {
					$.each(settings.icons, function(group, icons){
						$.each(icons, function(key, icon){
							$("#netliva_icon_modal ul").append('<li><i class="'+settings.groups[group].prefix+' netliva-demo-icon fa-'+icon+'" data-group="'+settings.groups[group].prefix+'" data-icon="'+icon+'"></i><b class="d-none">'+icon+'</b></li>');
						});
					});
					// Filtre
					$('#netlivaIconFinderInput').keyup(function()
					{
						var searchText = $(this).val();
						$('#netliva_icon_modal ul > li').each(function()
						{
					...
					$("#netliva_icon_modal li i").click(function(){
						$form_row.find("input").val($(this).data("group")+" fa-"+$(this).data("icon"));
						$form_row.find("i.netliva-demo-icon").removeClass().addClass("netliva-demo-icon "+$(this).data("group")+" fa-"+$(this).data("icon"));
						$("#netliva_icon_modal").modal("hide");
					});
				},
				buttons: [
	...
	function init()
	{
		$("[netliva_icon_field]").each(function () {
			if ($(this).attr("netliva_icon_field") !== "OK")
			{
				$(this).attr("netliva_icon_field", "OK");
				$(this).netlivaIconField();
			}
		});
	}
